...
namespace Integrant.Web.Pages
{
    public partial class Index
    {
        ...
        // private StructureState<User>       _structure    = null!;
        private FlareSelector<string> _tagsSelector = null!;
        private User                  _testUser     = null!;
        private StructureState<User>          _structureState = null!;
        ...
        protected override void OnInitialized()
        {
            ...
            // _structure = new StructureState<User>(validator: (structure, value) =>
            // {
            ...
            _structureState = Common.UserStructure.NewState();

            Common.UserStructure.GetMember<string>("Name").OnInputDebouncedValue +=
                (s, v, m) => Console.WriteLine($"StructureState<User>." + v.ID + " -> " + v);

            Common.UserStructure.GetMember<string>("Email").OnInputDebouncedValue +=
                (s, v, m) => Console.WriteLine($"StructureState<User>." + v.ID + " -> " + v);

            _structureState.OnMemberValueUpdate += (s, v, m) =>
            {
                Console.WriteLine($"StructureState<User>." + v.ID + " -> " + m);
            ...
            _tagsSelector.OnSelect += selected =>
                Common.UserStructure.GetMember<List<string>>(nameof(User.Tags))
                      .UpdateValue(_testUser, selected.Select(v => v.ID).ToList());

            _structureState.OnResetAllMemberInputs += () => _tagsSelector.Deselect();
            ...
            Task.Run(() =>
            {
                ...
                Common.UserStructure.GetMember<bool>(nameof(User.Boolean)).ResetInputs(_testUser);
                _structureState.Revalidate(_testUser);
                InvokeAsync(StateHasChanged);
            });
        }
        ...
        {
            _structureState.ResetAllMemberInputs(_testUser);
        }
    }
}